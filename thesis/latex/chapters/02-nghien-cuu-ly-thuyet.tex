...
tảng dùng để xây dựng hệ thống (mục 2.1 đến 2.4) và nhóm cơ sở thuật toán dùng
để giải các bài toán nghiệp vụ (mục 2.5). Với mỗi công nghệ, chương chỉ rõ luôn
công nghệ đó được vận dụng vào đâu trong đồ án để phần thiết kế ở
Chương~\ref{ch:hien-thuc} có căn cứ tham chiếu trực tiếp.
...
\subsection{Vai trò của bộ ánh xạ đối tượng quan hệ và cơ chế migrations}

Entity Framework Core (EF Core) là bộ ánh xạ đối tượng - quan hệ cho .NET, cho
phép thao tác với cơ sở dữ liệu quan hệ thông qua các đối tượng .NET thay vì
viết câu lệnh SQL trực tiếp. Ngoài lợi ích về năng suất, cách tiếp cận này còn
mang lại một lợi ích về an toàn thông tin thường bị bỏ qua: truy vấn viết bằng
LINQ được EF Core dịch thành câu lệnh SQL có tham số hóa, nên giá trị do người
dùng nhập không bao giờ được ghép trực tiếp vào chuỗi lệnh; nguy cơ chèn mã SQL
được loại bỏ ngay tại tầng truy cập dữ liệu chứ không phụ thuộc vào kỷ luật của
từng lập trình viên.
...
\subsection{Phân biệt xác thực, phân quyền và cách áp dụng}
...
mình. Từ đó có hai mức phân quyền: \textbf{mức chức năng} kiểm tra người dùng có
được phép gọi một hành động hay không, thường cài đặt bằng thuộc tính khai báo
trên Controller; \textbf{mức bản ghi} kiểm tra người dùng có được thao tác trên
\textit{đúng bản ghi đang xét} hay không, mức này phải đưa điều kiện lọc vào
chính câu truy vấn.

Đồ án định nghĩa hai vai trò là \code{Admin} và \code{User}. Toàn bộ khu vực
quản trị được bảo vệ bằng \code{[Authorize(Roles = "Admin")]}, đó là phân quyền
mức chức năng. Ở mức bản ghi, các thao tác trên thực đơn và danh sách đi chợ đều
kiểm tra bản ghi có thuộc về đúng người dùng đang đăng nhập hay không, và phép
kiểm tra này được đặt ngay trong biểu thức truy vấn chứ không đặt trong một câu
lệnh điều kiện sau khi đã nạp dữ liệu (mục~\ref{sec:thuat-toan-di-cho}).
...
và $M = 3$ bữa, tức 21 suất ăn, thỏa mãn một \textbf{ràng buộc cứng} là món phải
thuộc vùng miền người dùng chọn (nếu có), hai \textbf{ràng buộc mềm} là món phải
hợp bữa được gán và hạn chế lặp món trong tuần, cùng \textbf{tiêu chí tối ưu} là
tổng năng lượng trong ngày tiệm cận mức mục tiêu. Bài toán tối ưu toàn cục với
đầy đủ các ràng buộc đó thuộc lớp bài toán khó, nhưng với quy mô thực tế của đề
tài (21 suất, vài chục món), lời giải tối ưu tuyệt đối không phải yêu cầu bắt
buộc: người dùng chỉ cần một thực đơn hợp lý và luôn có thể chỉnh sửa thủ công.
...
Có hai điểm cần lưu ý về mặt thiết kế. \textbf{Thứ nhất}, khóa gộp nhóm phải là
cặp (nguyên liệu, đơn vị) chứ không chỉ là nguyên liệu: cùng một nguyên liệu có
thể được định lượng bằng đơn vị khác nhau ở những công thức khác nhau, ví dụ
hành lá tính theo gam ở món này nhưng theo bó ở món khác, nên cộng dồn hai giá
trị khác đơn vị sẽ cho kết quả vô nghĩa. \textbf{Thứ hai}, phép duyệt phải theo
suất ăn chứ không theo món: nếu một món xuất hiện ba lần trong tuần thì nguyên
liệu của nó phải được tính ba lần.
