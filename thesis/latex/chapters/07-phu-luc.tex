...
Developer, bật chế độ xác thực hỗn hợp và dùng tài khoản \code{sa} hoặc tạo login
riêng.

\textbf{Cách C, LocalDB (Windows, nhẹ nhất cho demo).} Có sẵn khi cài Visual
Studio.
...
\toprule
\bfseries Môi trường & \bfseries Chuỗi kết nối \\
\midrule
Docker (macOS hoặc Windows) &
  \codelong{Server=localhost,1433;Database=CookingAdvisor;User Id=sa;
  Password=CookAdvisor@2026;TrustServerCertificate=True} \\
\rowhl SQL Server cài trực tiếp (Windows) &
  \codelong{Server=localhost;Database=CookingAdvisor;User Id=sa;
  Password=<mật khẩu của bạn>;TrustServerCertificate=True} \\
LocalDB (Windows) &
  \codelong{Server=(localdb)\textbackslash MSSQLLocalDB;Database=CookingAdvisor;
  Trusted\_\allowbreak{}Connection=True;TrustServerCertificate=True} \\
\bottomrule
\end{tabularx}
\end{table}
...
\toprule
\bfseries Vai trò & \bfseries Email & \bfseries Mật khẩu \\
\midrule
Quản trị viên & \codelong{admin@cookingadvisor.local} & \code{Admin@2026!Cook} \\
\bottomrule
\end{tabularx}
\end{table}
...
\toprule
\bfseries Món ăn & \bfseries Tác giả & \bfseries Giấy phép \\
\midrule
Phở bò & Andy Li & CC0 \\
\rowhl Bún chả & Phương Huy & Public Domain \\
Bún bò Huế & Baoothersks & CC BY-SA 4.0 \\
\rowhl Cơm tấm thịt nướng & Tokeisan & Public Domain \\
Gỏi cuốn & Tran Hai Duong & CC0 \\
\rowhl Chả giò & Satdeep Gill & CC BY-SA 4.0 \\
Bánh xèo & Orderinchaos & CC BY-SA 4.0 \\
\rowhl Rau muống xào tỏi & Andy Wright & CC BY 2.0 \\
\bottomrule
\end{tabularx}
\end{table}
...
Trích từ \codelong{src/CookingAdvisor/Services/SuggestionService.cs}:
...
Trích từ \codelong{src/CookingAdvisor/Services/MenuPlannerService.cs}. Đây là đoạn
khắc phục khiếm khuyết suy thoái tính đa dạng đã trình bày ở
mục~\ref{sec:khiem-khuyet}:
...
Trích từ \codelong{src/CookingAdvisor/Services/ShoppingListService.cs}:
...
\unnumberedsubsection{D.4. Ca kiểm thử tiêu biểu}

Trích từ \codelong{src/CookingAdvisor.Tests/Services/RecipeServiceSortTests.cs}. Ca
kiểm thử này đã được xác nhận là có hiệu lực bằng cách tạm gỡ khóa sắp xếp phụ
khỏi mã nguồn và quan sát ca kiểm thử chuyển sang trạng thái không đạt:
...
public async Task Breaks_ties_by_name_so_paging_cannot_repeat_a_dish()
{
    ...
    // and page 1 would start at "Mon 12" instead of "Mon 01".
    var recipes = Enumerable.Range(1, RecipeService.PageSize + 3)
        .Select(i => BuildRecipe(i, $"Mon {i:D2}", 10, 10, 500))
        .Reverse()
        .ToArray();
    ...
    Assert.Equal(
        Enumerable.Range(1, RecipeService.PageSize).Select(i => $"Mon {i:D2}"),
