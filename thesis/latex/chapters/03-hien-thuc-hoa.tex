...
\code{Id} & int, IDENTITY & Khóa chính, tự tăng \\ \hline
\code{Name} & nvarchar, NOT NULL & Tên danh mục, ví dụ Món canh, Món
  xào \\ \hline
\end{tabularx}
\end{table}
...
\code{Unit} & nvarchar, NOT NULL & Đơn vị đo mặc định của nguyên liệu \\ \hline
\code{Group} & nvarchar, NOT NULL & Nhóm nguyên liệu, ví dụ Rau củ, Thịt,
  Gia vị \\ \hline
\end{tabularx}
\end{table}
...
\code{CaloriesPerServing} & int & Năng lượng mỗi khẩu phần, kcal \\ \hline
\code{ImageUrl} & nvarchar, NULL & Đường dẫn ảnh minh họa \\ \hline
\code{SuitableMealTypes} & int & Cờ nhị phân các bữa phù hợp \\ \hline
...
\code{RecipeId} & int, PK, FK & Khóa chính thành phần, khóa ngoại tới
  \code{Recipes} \\ \hline
\code{IngredientId} & int, PK, FK & Khóa chính thành phần, khóa ngoại tới
...
\bfseries Trường & \bfseries Kiểu & \bfseries Mô tả \\ \hline
\code{Id} & int, IDENTITY & Khóa chính, tự tăng \\ \hline
\code{UserId} & nvarchar, FK & Khóa ngoại tới bảng người dùng của
  Identity \\ \hline
...
\bfseries Trường & \bfseries Kiểu & \bfseries Mô tả \\ \hline
\code{Id} & int, IDENTITY & Khóa chính, tự tăng \\ \hline
\code{MenuPlanId} & int, FK, UNIQUE & Khóa ngoại tới \code{MenuPlans},
  có ràng buộc duy nhất \\ \hline
...
\code{Quantity} & decimal(10,2) & Tổng khối lượng cần mua sau khi cộng
  dồn \\ \hline
...
\endfoot
Tác nhân & Người dùng \\ \hline
Mục tiêu & Thay món hoặc bỏ trống một ô trong lưới thực đơn \\ \hline
...
Luồng chính & Người dùng mở chi tiết thực đơn, chọn món khác từ hộp chọn trong ô
  ...
  năng lượng của ngày đó và hiển thị lại lưới \\ \hline
Luồng thay thế & Nếu mã thực đơn không thuộc về người dùng, hệ thống trả về lỗi
  ...
  giữ trong lưới để người dùng gán món trở lại \\ \hline
Hậu điều kiện & Bản ghi \code{MenuPlanItem} tương ứng được cập nhật hoặc xóa;
  danh sách đi chợ đã sinh trước đó có thể tạo lại để phản ánh thay đổi \\ \hline
\end{longtable}
...
\endfoot
Tác nhân & Người dùng \\ \hline
Mục tiêu & Theo dõi tiến độ mua sắm khi đang đi chợ \\ \hline
Tiền điều kiện & Danh sách đi chợ đã tồn tại và thuộc về người dùng \\ \hline
Luồng chính & Người dùng tick vào ô của một dòng nguyên liệu; hệ thống đảo trạng
  thái đã mua của dòng đó, gạch ngang dòng và cập nhật thanh tiến độ \\ \hline
Luồng thay thế & Nếu dòng không thuộc danh sách của người dùng, hệ thống trả về
  lỗi không tìm thấy \\ \hline
Hậu điều kiện & Cột \code{IsPurchased} của dòng tương ứng được cập nhật \\ \hline
\end{longtable}
...
\endfoot
Tác nhân & Khách, Người dùng, Quản trị viên \\ \hline
Mục tiêu & Tìm ra món cần xem trong kho dữ liệu \\ \hline
Tiền điều kiện & Không có \\ \hline
...
Luồng thay thế & Nếu không có món nào thỏa điều kiện, hệ thống hiển thị trạng
  ...
  không hợp lệ, hệ thống dùng giá trị mặc định \\ \hline
...
\endfoot
Tác nhân & Khách, Người dùng, Quản trị viên \\ \hline
Mục tiêu & Xem đầy đủ công thức, nguyên liệu định lượng và các bước nấu \\ \hline
Tiền điều kiện & Món ăn tồn tại trong cơ sở dữ liệu \\ \hline
Luồng chính & Người dùng bấm vào một món trong lưới; hệ thống nạp món kèm danh
  ...
  và các bước nấu \\ \hline
Luồng thay thế & Nếu mã món không tồn tại, hệ thống trả về trang không tìm thấy.
  Nếu người dùng đã đăng nhập, trang hiển thị thêm nút thêm hoặc bỏ yêu thích \\ \hline
...
\endhead
\endfoot
Tác nhân & Khách \\ \hline
Mục tiêu & Tạo tài khoản để dùng nhóm chức năng dành cho Người dùng \\ \hline
Tiền điều kiện & Chưa đăng nhập \\ \hline
Luồng chính & Khách nhập họ tên, thư điện tử, mật khẩu và xác nhận mật khẩu; hệ
  ...
  \code{User} rồi đăng nhập luôn cho người dùng \\ \hline
Luồng thay thế & Nếu thư điện tử sai định dạng hoặc đã tồn tại, mật khẩu ngắn
  ...
  Việt ngay dưới ô nhập tương ứng và không tạo tài khoản \\ \hline
Hậu điều kiện & Tài khoản mới tồn tại với vai trò \code{User} \\ \hline
\end{longtable}
...
\endfoot
Tác nhân & Khách, Người dùng, Quản trị viên \\ \hline
Mục tiêu & Xác thực danh tính để mở khóa nhóm chức năng tương ứng với vai trò \\ \hline
...
Luồng chính & Người dùng nhập thư điện tử và mật khẩu; hệ thống xác thực, tạo
  ...
  hệ thống hủy phiên và quay về trang chủ \\ \hline
Luồng thay thế & Nếu thông tin đăng nhập sai, hệ thống báo lỗi chung mà không
  tiết lộ sai ở trường nào \\ \hline
Hậu điều kiện & Phiên đăng nhập được tạo hoặc hủy tương ứng \\ \hline
\end{longtable}
...
Mục tiêu & Duy trì kho dữ liệu món ăn cùng danh sách nguyên liệu định lượng \\ \hline
Tiền điều kiện & Đã đăng nhập bằng tài khoản thuộc vai trò \code{Admin} \\ \hline
Luồng chính & Quản trị viên mở khu quản trị, chọn thêm hoặc sửa một món; nhập
  ...
  nguyên liệu kèm khối lượng và đơn vị rồi lưu \\ \hline
Luồng thay thế & Nếu dữ liệu không hợp lệ, biểu mẫu hiển thị lỗi và giữ nguyên
  ...
  Bảng~\ref{tab:hanh-vi-xoa} \\ \hline
...
\endfoot
Tác nhân & Quản trị viên \\ \hline
Mục tiêu & Duy trì danh mục món ăn và kho nguyên liệu dùng chung \\ \hline
Tiền điều kiện & Đã đăng nhập bằng tài khoản thuộc vai trò \code{Admin} \\ \hline
Luồng chính & Quản trị viên xem danh sách, thêm mới, sửa hoặc xóa một nguyên
  liệu hay một danh mục; hệ thống lưu thay đổi và quay lại danh sách \\ \hline
Luồng thay thế & Nếu nguyên liệu đang được dùng trong công thức hoặc đang nằm
  ...
  giải thích. Tương tự với danh mục đang có món sử dụng \\ \hline
Hậu điều kiện & Bảng \code{Ingredients} hoặc \code{Categories} được cập nhật mà
  không phá vỡ toàn vẹn tham chiếu \\ \hline
\end{longtable}
